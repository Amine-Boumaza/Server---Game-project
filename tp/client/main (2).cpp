 ...
 * programme écrit par le Dr. Brahimi said                                              *
 * chargé de module Réseaux de communication - 2eme année licence en informatique       *
 * université 8 mai 1945 - Guelma (univ-guelma.dz)                                      *
 *-------------------------------------------------------------------------------------**/
...
#define ADD_IP_SERVER  "127.0.0.1"    // à changer en cas de réseau
#define PORT 1977
...
/****************************************************************************************
 *                créer un socket qui sert d'un canal logique entre                     *
...
int se_deconnecter_de_serveur(SOCKET sockt){
    ...
    if ( retour == SOCKET_ERROR){
        printf("Probleme de fermméture de socket \n");
        return 0;
        }
    return 1;
}
/*-*************************************************************************************/
int se_connecter_avec_serveur(SOCKET *sockt, const char add_IP[], const u_short n_port){  /*add_IP est l'adresse IP du serveur
                                                                et n_port est son n° de port */
    /* déclarer la structure qui va spécifier les cordonées(add ip et n° de port de serveur; */
    struct sockaddr_in  cordonees_de_serveur;

    /* créer socket de commmunication avec le serveur  */
    if( creation_de_socket(sockt) == 0) return 0;

    /* préparer la structure de cordonnées qui va être utiliser pour se connecter avec le serveur   */
    cordonees_de_serveur.sin_family = AF_INET;
    cordonees_de_serveur.sin_addr.s_addr = inet_addr(add_IP); /* préciser l'adresse IPv4 de serveur */
    cordonees_de_serveur.sin_port = htons(n_port);            /* préciser le numéro de port de serveur */

    // étalissement de connection avec le processus distant (serveur)
...
int envoyer_message_au_serveur(SOCKET sockt, char message_a_envoyer[]){
    int ecrits = send(sockt, message_a_envoyer , (int)strlen(message_a_envoyer), 0); /* message à TAILLE variable */
...
int recevoir_message_du_serveur(SOCKET sockt, char *buffer ){  /* Reception des données du serveur */
    ...
    if( lus > 0 ){ /* réception de lus octets */
        //printf("          Reponse de serveur : \n%s \n", message_a_recevoir);
        strcpy(buffer, message_a_recevoir) ;
        return 1 ;
        }
    if ( lus == 0 ) {/* la socket est fermée par le serveur */
        printf("Serveur deconnecte (Socket est fermée par le serveur) !\n");
        //est_connecte = 0 ;
        return 0 ;
        }
    else {/* une erreur ! */
...
void extraire_deplacement(char* message_a_recevoir, int* xs, int* ys, int* xc, int* yc) {
  ...
  *xs=0 ; *ys=0 ; *xc=0; *yc=0;
  if((strlen(message_a_recevoir)==7)&(message_a_recevoir[1]==' ')&(message_a_recevoir[3]==' ')
     ...
     if(*xc!=0)
     {
        if(message_a_recevoir[2]=='1')      *yc=1 ;
        else if(message_a_recevoir[2]=='2') *yc=2 ;
        else if(message_a_recevoir[2]=='3') *yc=3 ;
     }
   }
}
...
void mettre_a_jeur_gril(char type, int x, int y){
  /** A compléter
      ici on met à jour la gril local
...
int initialisation_du_jeu(){
    ...
    if(etat_connexion == 0) return 0;   /* quitter l'exécution, il y a un problème de connexion échoue */
    /* envoyer un message au serveur */
    etat_envoi = envoyer_message_au_serveur(socketDialogue, NomJoueur);
    if( etat_envoi == 0 ) return 0;  /* quitter l'exécution, il y a un problème lors de l'envoi du message */
    /* recevoir un message de serveur */
    etat_reception = recevoir_message_du_serveur(socketDialogue, message_a_recevoir) ;
    if( etat_reception == 0 ) return 0; /* quitter l'exécution, il y a un probleme de communication  */
...
/*-**************************************************************************************/
 ...
 int placement_des_pieces(){
     ...
     /** A compléter
      ici c'est la phase de placement des pièces
      le joueur propose un placement et met à jour la gril local après
      ...
      - la reconnaissance de l'action du jeu de l'adverssaire (selon
        l'information retounrnée par le serveur) (on Mette O)

     retourne :
     0 : en cas de problème de communication
     1 : si il arrive à bien positionner sa 3 eme pièce
     2 : le joueur arrive à aligner correctement ses trois pièces, ce joueur est alor vinqueur
     3 : l'adversaire arrive à aligner ces 3 pièce, ou le joueur n'arrive pas
         à exprimmer en trois tentive une placement correcte.
         il est alor vinqueur et ce joueur échoue
    ...
    //         if(etat_reception==0) return 0;
         ...
         else if(strcmp(message_a_recevoir,TU_A_GAGNE)==0) return 2;
         else if(strcmp(message_a_recevoir,ADVERSAIRE_DECONNECTE)==0) return 4;
         else {
...
int deplacement_des_pieces(){
     /** A compléter
      ici c'est la phase de deplacement des pièces
      le joueur propose à chaque fois un deplacement et met à jour la gril local après
      - la cofirmation de son action (de deplacement local) par serveur
      - la reconnaissance de l'action du jeu de l'adverssaire (selon
        l'information retounrnée par le serveur)

     retourne :
     0 : en cas de problème de communication
     2 : le joueur arrive à aligner correctement ses trois pièces, ce joueur est alor vinqueur
     3 : l'adversaire arrive à aligner ces 3 pièce, ou le joueur n'arrive pas
         à exprimmer en trois tentive un deplacement correcte.
         il est alor vinqueur et ce joueur échoue
     4 : ADVERSAIRE DECONNECTE
     **/
     char message_a_recevoir[LG_MESSAGE],message_a_envoyer[LG_MESSAGE];
...
/****************************************************************************************/
 ...
 {  int resultat=-1;
 char corbeil;
    init_bib();     // initialiser la bibliothèque "WinSock"
    while(resultat!= 0)
    {
        ...
        if(resultat!=1) afficher_scord(resultat);
        else
        {
    ...
    fermer_bib() ;  // fermmer la bibliothèque "WinSock"
    //scanf("%d", &resultat);
    return 1;
 }
/****************************************************************************************/
